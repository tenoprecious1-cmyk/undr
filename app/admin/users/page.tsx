      ...
      <div className="mt-6 flex flex-col gap-2">
        {users.map((u) => {
          ...
          return (
            <div
              key={u.id}
              className="flex flex-wrap items-center gap-3 rounded-2xl border border-border-soft bg-surface/40 p-3"
            >
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-accent-soft text-lg">
                {u.emoji}
              </span>
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-1.5 truncate text-sm font-semibold text-text">
                  {identityHandle(u)}
                ...
                <>
                  ...
                  {userRoles.map((rk) => (
                    <form key={rk} action={adminRevokeRoleAction.bind(null, u.id, rk)}>
                      <button
                        type="submit"
                        className="rounded-full border border-border-soft px-3 py-1.5 text-xs font-semibold text-text-dim transition hover:border-danger/40 hover:text-danger"
                      >
                        Remove {rk.replace("_", " ")}
                      </button>
                    </form>
